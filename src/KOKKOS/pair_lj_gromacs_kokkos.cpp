...
void PairLJGromacsKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  ...
  EV_FLOAT ev = pair_compute<PairLJGromacsKokkos<DeviceType>,void >(this,(NeighListKokkos<DeviceType>*)list);
  DeviceType::fence();
...
compute_fpair(const F_FLOAT& rsq, const int& i, const int&j, const int& itype, const int& jtype) const {
  ...
  const r_cut_inner = (STACKPARAMS?m_params[itype][jtype].cut_inner_sq:params(itype,jtype).cut_inner_sq);
  if(rsq > r_cut_inner) {
    const F_FLOAT r = sqrt(rsq);
    const F_FLOAT t = r - r_cut_inner;
    const F_FLOAT fswitch = r*t*t*
      ((STACKPARAMS?m_param[itype][jtype].ljsw1:params(itype,jtype).ljsw1) +
       (STACKPARAMS?m_params[itype][jtype].ljsw2:params(itype,jtype).ljsw2*t));
    return (forcelj+fswitch)*r2inv;
  } else {
...
compute_evdwl(const F_FLOAT& rsq, const int& i, const int&j, const int& itype, const int& jtype) const {
  ...
  const F_FLOAT elj = r6inv*((STACKPARAMS?m_params[itype][jtype].lj3:params(itype,jtype).lj3)*r6inv -
                               (STACKPARAMS?m_params[itype][jtype].lj4:params(itype,jtype).lj4)) +
    (STACKPARAMS?m_params[itype][jtype].ljsw5:params(itype,jtype).ljsw5);
  ...
  if(rsq > r_cut_inner) {
    ...
    const F_FLOAT eswitch = t*t*t*
      ((STACKPARAMS?m_param[itype][jtype].ljsw3:params(itype,jtype).ljsw3) +
       (STACKPARAMS?m_params[itype][jtype].ljsw4:params(itype,jtype).ljsw4*t));
    return (elj+eswitch);
  } else {
    return elj;
  }
