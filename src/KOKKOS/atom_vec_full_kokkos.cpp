...
struct AtomVecFullKokkos_UnpackBorder {
  ...
  void operator() (const int& i) const {
      ...
      _molecule(i+_first) = static_cast<int> (_buf(i,7));
                                                 ...
                                                 X_FLOAT hi )
{
  const size_t elements = 20+atom->maxspecial+2*atom->bond_per_atom+4*atom->angle_per_atom+
      5*atom->dihedral_per_atom + 5*atom->improper_per_atom;
  if(nsend > (k_buf.view<LMPHostType>().dimension_0()*
...
int AtomVecFullKokkos::pack_exchange(int i, double *buf)
{
  ...
  buf[m++] = h_v(i,1);
  buf[m++] = h_v(i,2);
  *((tagint *) &buf[m++]) = h_tag(i);
  buf[m++] = h_type(i);
  buf[m++] = h_mask(i);
  *((imageint *) &buf[m++]) = h_image(i);
  buf[m++] = h_q(i);
  *((tagint *) &buf[m++]) = h_molecule(i);
  buf[m++] = h_num_bond(i);
  for (k = 0; k < h_num_bond(i); k++) {
    buf[m++] = h_bond_type(i,k);
    *((tagint *) &buf[m++]) = h_bond_atom(i,k);
  }
  buf[m++] = h_num_angle(i);
  for (k = 0; k < h_num_angle(i); k++) {
    buf[m++] = h_angle_type(i,k);
    *((tagint *) &buf[m++]) = h_angle_atom1(i,k);
    *((tagint *) &buf[m++]) = h_angle_atom2(i,k);
    *((tagint *) &buf[m++]) = h_angle_atom3(i,k);
  }
  buf[m++] = h_num_dihedral(i);
  for (k = 0; k < h_num_dihedral(i); k++) {
    buf[m++] = h_dihedral_type(i,k);
    *((tagint *) &buf[m++]) = h_dihedral_atom1(i,k);
    *((tagint *) &buf[m++]) = h_dihedral_atom2(i,k);
    *((tagint *) &buf[m++]) = h_dihedral_atom3(i,k);
    *((tagint *) &buf[m++]) = h_dihedral_atom4(i,k);
  }
  buf[m++] = h_num_improper(i);
  for (k = 0; k < h_num_improper(i); k++) {
    buf[m++] = h_improper_type(i,k);
    *((tagint *) &buf[m++]) = h_improper_atom1(i,k);
    *((tagint *) &buf[m++]) = h_improper_atom2(i,k);
    *((tagint *) &buf[m++]) = h_improper_atom3(i,k);
    *((tagint *) &buf[m++]) = h_improper_atom4(i,k);
  }
  buf[m++] = h_nspecial(i,0);
  buf[m++] = h_nspecial(i,1);
  buf[m++] = h_nspecial(i,2);
  for (k = 0; k < h_nspecial(i,2); k++)
    *((tagint *) &buf[m++]) = h_special(i,k);
...
int AtomVecFullKokkos::unpack_exchange(double *buf)
{
  ...
  h_v(nlocal,1) = buf[m++];
  h_v(nlocal,2) = buf[m++];
  h_tag(nlocal) = *((tagint *) &buf[m++]);
  h_type(nlocal) = static_cast<int> (buf[m++]);
  h_mask(nlocal) = static_cast<int> (buf[m++]);
  h_image(nlocal) = *((imageint *) &buf[m++]);
  h_q(nlocal) = buf[m++];
  h_molecule(nlocal) = *((tagint *) &buf[m++]);

  h_num_bond(nlocal) = static_cast<int> (buf[m++]);
  for (k = 0; k < h_num_bond(nlocal); k++) {
    h_bond_type(nlocal,k) = static_cast<int> (buf[m++]);
    h_bond_atom(nlocal,k) = *((tagint *) &buf[m++]);
  }
  h_num_angle(nlocal) = static_cast<int> (buf[m++]);
  for (k = 0; k < h_num_angle(nlocal); k++) {
    h_angle_type(nlocal,k) = static_cast<int> (buf[m++]);
    h_angle_atom1(nlocal,k) = *((tagint *) &buf[m++]);
    h_angle_atom2(nlocal,k) = *((tagint *) &buf[m++]);
    h_angle_atom3(nlocal,k) = *((tagint *) &buf[m++]);
  }
  h_num_dihedral(nlocal) = static_cast<int> (buf[m++]);
  for (k = 0; k < h_num_dihedral(nlocal); k++) {
    h_dihedral_type(nlocal,k) = static_cast<int> (buf[m++]);
    h_dihedral_atom1(nlocal,k) = *((tagint *) &buf[m++]);
    h_dihedral_atom2(nlocal,k) = *((tagint *) &buf[m++]);
    h_dihedral_atom3(nlocal,k) = *((tagint *) &buf[m++]);
    h_dihedral_atom4(nlocal,k) = *((tagint *) &buf[m++]);
  }
  h_num_improper(nlocal) = static_cast<int> (buf[m++]);
  for (k = 0; k < h_num_improper(nlocal); k++) {
    h_improper_type(nlocal,k) = static_cast<int> (buf[m++]);
    h_improper_atom1(nlocal,k) = *((tagint *) &buf[m++]);
    h_improper_atom2(nlocal,k) = *((tagint *) &buf[m++]);
    h_improper_atom3(nlocal,k) = *((tagint *) &buf[m++]);
    h_improper_atom4(nlocal,k) = *((tagint *) &buf[m++]);
  }
  h_nspecial(nlocal,0) = static_cast<int> (buf[m++]);
  h_nspecial(nlocal,1) = static_cast<int> (buf[m++]);
  h_nspecial(nlocal,2) = static_cast<int> (buf[m++]);
  for (k = 0; k < h_nspecial(nlocal,2); k++)
	 h_special(nlocal,k) = *((tagint *) &buf[m++]);
...
int AtomVecFullKokkos::pack_restart(int i, double *buf)
{
  ...
  buf[m++] = h_num_bond(i);
  for (k = 0; k < h_num_bond(i); k++) {
    buf[m++] = MAX(h_bond_type(i,k),-h_bond_type(i,k));
    buf[m++] = h_bond_atom(i,k);
  }

  buf[m++] = h_num_angle(i);
  for (k = 0; k < h_num_angle(i); k++) {
    buf[m++] = MAX(h_angle_type(i,k),-h_angle_type(i,k));
    buf[m++] = h_angle_atom1(i,k);
    buf[m++] = h_angle_atom2(i,k);
    buf[m++] = h_angle_atom3(i,k);
  }

  buf[m++] = h_num_dihedral(i);
  for (k = 0; k < h_num_dihedral(i); k++) {
    buf[m++] = MAX(h_dihedral_type(i,k),-h_dihedral_type(i,k));
    buf[m++] = h_dihedral_atom1(i,k);
    buf[m++] = h_dihedral_atom2(i,k);
    buf[m++] = h_dihedral_atom3(i,k);
    buf[m++] = h_dihedral_atom4(i,k);
  }

  buf[m++] = h_num_improper(i);
  for (k = 0; k < h_num_improper(i); k++) {
    buf[m++] = MAX(h_improper_type(i,k),-h_improper_type(i,k));
    buf[m++] = h_improper_atom1(i,k);
    buf[m++] = h_improper_atom2(i,k);
    buf[m++] = h_improper_atom3(i,k);
    buf[m++] = h_improper_atom4(i,k);
  }
...
{
  fprintf(fp," %-1.16e" TAGINT_FORMAT, (tagint) (buf[0]),buf[1]);
  return 2;
}
