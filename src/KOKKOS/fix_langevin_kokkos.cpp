  ...
  FixLangevin(lmp, narg, arg),rand_pool(seed + comm->me)
{
  ...
  d_gfactor1 = k_gfactor1.view<DeviceType>();
  h_gfactor1 = k_gfactor1.view<LMPHostType>();
  d_gfactor2 = k_gfactor2.view<DeviceType>();
  h_gfactor2 = k_gfactor2.view<LMPHostType>();
  d_ratio = k_ratio.view<DeviceType>();
  h_ratio = k_ratio.view<LMPHostType>();

  // optional args
  for (int i = 1; i <= ntypes; i++) ratio[i] = 1.0;
  k_ratio.modify<LMPHostType>();
  ...
  if(gjfflag){
    ...
    k_franprev.modify<LMPHostType>();
  }
  if(zeroflag){
    k_fsumall = tdual_double_1d_3n("langevin:fsumall");
    h_fsumall = k_fsumall.view<LMPHostType>();
    d_fsumall = k_fsumall.view<DeviceType>();
  }
...
void FixLangevinKokkos<DeviceType>::init()
{
  ...
  k_gfactor1.modify<LMPHostType>();
  k_gfactor2.modify<LMPHostType>();
}
...
void FixLangevinKokkos<DeviceType>::grow_arrays(int nmax)
{
  ...
  d_franprev = k_franprev.view<DeviceType>();
  h_franprev = k_franprev.view<LMPHostType>();
}
...
void FixLangevinKokkos<DeviceType>::post_force(int vflag)
{
  ...
  k_gfactor1.sync<DeviceType>();
  k_gfactor2.sync<DeviceType>();
  k_ratio.sync<DeviceType>();
  if(gjfflag) k_franprev.sync<DeviceType>();
  ...
  if (tallyflag) {
    if (nlocal > maxatom1) {
      ...
      d_flangevin = k_flangevin.view<DeviceType>();
      h_flangevin = k_flangevin.view<LMPHostType>();
    }
  }
  ...
  if (gjfflag) k_franprev.modify<DeviceType>();
  if (tallyflag) k_flangevin.modify<DeviceType>();
  ...
  if (zeroflag) {
    ...
    k_fsumall.modify<LMPHostType>();
    k_fsumall.sync<DeviceType>();
  ...
  if (tstyle == CONSTANT) {
    ...
    if (tstyle == EQUAL) {
      ...
      if (nlocal > maxatom2) {
        ...
        d_tforce = k_tforce.view<DeviceType>();
        h_tforce = k_tforce.view<DeviceType>();
      }
      input->variable->compute_atom(tvar,igroup,tforce,1,0); // tforce is modified on host
      k_tforce.modify<LMPHostType>();
