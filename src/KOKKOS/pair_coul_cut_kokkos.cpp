...
compute_fcoul(const F_FLOAT& rsq, const int& i, const int&j, const int& itype, const int& jtype) const {
...
KOKKOS_INLINE_FUNCTION
F_FLOAT PairCoulCutKokkos<DeviceType>::
compute_ecoul(const F_FLOAT& rsq, const int& i, const int&j, const int& itype, const int& jtype) const {
  const F_FLOAT r2inv = 1.0/rsq;
  const F_FLOAT rinv = sqrt(r2inv);

  return qqrd2e * (STACKPARAMS?m_params[itype][jtype].scale:params(itype,jtype).scale)
    * q(i)*q(j)*rinv;
}
...
void PairCoulCutKokkos<DeviceType>::allocate()
{
  ...
  k_cut_ljsq = Kokkos::DualView<double **,Kokkos::LayoutRight,DeviceType>("pair:cut_ljsq",n+1,n+1);
  d_cut_ljsq = k_cut_ljsq.template view<DeviceType>();
  k_cut_coulsq = Kokkos::DualView<double **,Kokkos::LayoutRight,DeviceType>("pair:cut_coulsq",n+1,n+1);
